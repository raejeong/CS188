...
def fillObsCPT(bayesNet, gameState):
    ...
    yFactor = bn.Factor([Y_POS_VAR], [], bayesNet.variableDomainsDict())
    HOUSE_VAL_DICT = dict(zip(gameState.getPossibleHouses(), [HOUSE_VALS[i] for i in [2, 0, 3, 1]]))
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            ...
            obsLocation = 0
            for houseObs in gameState.getPossibleHouses():
                if obsPos in gameState.getHouseWalls(houseObs):
                    obsLocation = HOUSE_VAL_DICT[houseObs]
            for row in obsCPT:
                ...
                if obsLocation != ghostHouseVal and obsLocation != foodHouseVal:
                    if color == RED_OBS_VAL:
                        ...
                        prob = 1
                elif ghostHouseVal == foodHouseVal or foodHouseVal == obsLocation:
                    if color == RED_OBS_VAL:
                        prob = PROB_FOOD_RED
                    elif color == BLUE_OBS_VAL:
                        ...
                        prob = 0
                elif obsLocation == ghostHouseVal:
                    if color == RED_OBS_VAL:
                        prob = PROB_GHOST_RED
                    elif color == BLUE_OBS_VAL:
                        ...
                        prob = 0
                obsFactor.setProbability(row, prob)
